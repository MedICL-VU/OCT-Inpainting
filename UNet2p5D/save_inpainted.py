...
def inpaint_volume_with_model_clipped(model, corrupted_volume, mask, device, stack_size=5):
    """
    Apply 2.5D model to missing slices in a volume.
    ...
    with torch.no_grad():
        for idx in np.where(mask == 1)[0]:
            ...
            pred = pred * corrupted_volume.max()
            inpainted[idx] = np.clip(pred, 0, corrupted_volume.max()).astype(np.uint16)

    return inpainted


def inpaint_volume_with_model_recursive(model, corrupted_volume, mask, device, stack_size=5):
    """
    Apply 2.5D model to missing slices in a volume.
        ...
        for block in corrupted_blocks:
            ...
            while left <= right:
                for idx in [block[left], block[right]] if left != right else [block[left]]:
                    ...
                    output = model(stack)
                    pred = output.squeeze().cpu().numpy() * corrupted_volume.max()
                    reconstructed_volume[idx] = np.clip(pred, 0, corrupted_volume.max()).astype(np.uint16)
...
def inpaint_volume_with_model_CLIPPEDORIGINAL(model, corrupted_volume, mask, device, stack_size=5):
    """
    Apply 2.5D model to missing slices in a volume.
