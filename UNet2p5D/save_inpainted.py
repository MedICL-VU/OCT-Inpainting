...
def inpaint_volume_with_model(model, corrupted_volume, mask, device, stack_size=5):
    ...
    Reuses inpainted slices recursively during inference.
    ...
    # This will hold the progressively updated volume
    ...
    with torch.no_grad():
        for idx in np.where(mask == 1)[0]:
            ...
            # Write prediction into full and padded volumes
            reconstructed_volume[idx] = pred
            padded[idx + pad] = pred  # also update padded volume for next iterations
